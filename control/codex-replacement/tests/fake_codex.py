#!/usr/bin/env python3
"""Provider-free fake for the replacement-instructions natural-turn contract."""
...
def replacement_instruction() -> tuple[str, dict[str, object], Path]:
    codex_home = Path(os.environ["CODEX_HOME"])
    config = tomllib.loads((codex_home / "config.toml").read_text(encoding="utf-8"))
    assert config.get("project_doc_max_bytes") == 0
    configured = config.get("model_instructions_file")
    assert configured == "model_instructions.md"
    path = Path(configured)
    if not path.is_absolute():
        path = codex_home / path
    forbidden = (
        Path.cwd() / "AGENTS.md",
        Path.cwd() / "AGENTS.override.md",
        codex_home / "AGENTS.md",
        codex_home / "AGENTS.override.md",
    )
    assert not any(item.exists() or item.is_symlink() for item in forbidden)
    return path.read_text(encoding="utf-8"), config, path
...
def main() -> int:
    ...
    codex_home = Path(os.environ["CODEX_HOME"])
    instruction, config, instruction_path = replacement_instruction()
    ...
    output(
        {
            ...
            "replacement_instruction_bytes": len(instruction.encode("utf-8")),
