...
- LLMs as Strong Interpreters Without Transmission Ecology
- Reader Constraint
...
- Necessary Conditions for Autonomous Recursive Accumulation
- Persistence-Plasticity Constraint
- Automating Labor Is Not Automating Selection
- What Would Count as Removing the Human Selector?
...
The target of this paper is not ordinary model improvement, better memory, or
AI-assisted research acceleration. The target is autonomous recursive
accumulation: a system in which structures generated inside the system become
inherited conditions for future instances, and are differentially preserved
because they improve future continuation.

This distinction matters because recursive self-improvement is often described
as a model improving its own training, architecture, tools, or successor. But
the known open-ended cumulative systems, biological evolution and human culture,
do not operate primarily as isolated self-modifiers. They operate through
heritable structure, transmission gaps, variation, selection, and reconstruction
by later instances.

The question is not whether AI can contribute to its own improvement. It already
can in limited ways. The question is whether the structures generated by AI can
become a self-sustaining hereditary ecology once human selection is removed.

The central question is therefore:

> What minimal architecture would allow AI-generated process structure to become
> heritable, selectable, and cumulative without relying on a privileged human
> curator at every cycle?

The practical architecture proposed later is an answer to this question. It is
not merely a memory architecture. It is an attempt to create the missing
selection-and-transmission layer.

This is not a proposal for giving LLMs more memory. It is a proposal for
turning externalized process into heritable culture by making future
fresh-context agents depend on bounded artifacts under scarcity, evaluation, and
differential continuation.

### Claims Ladder

The argument should be read as a ladder of claims rather than as a single
maximal assertion.

Claim 1: conceptual correction. Recursive improvement should not be modeled
primarily as isolated self-modification. The known cumulative systems are
hereditary and ecological.

Claim 2: functional requirement. Autonomous recursive accumulation requires a
stable interpreter, persistent substrate, transmission gap, heritable variation,
scarcity, protected selection, and differential continuation.

Claim 3: LLM diagnosis. LLMs already have strong interpreter machinery but weak
AI-native transmission ecology.

Claim 4: architecture. A four-channel architecture of constructor, workspace,
lineage seed, and archive can operationalize artificial cultural evolution.

Claim 5: empirical prediction. Under bounded context, costly attention, and
descendant-based selection, lineage seeds and archive artifacts should become
more compact, reconstructible, modular, and uptake-sensitive over generations.

Claim 6: near-term scope. The first empirical target is not full open-ended
recursive self-improvement, but domain-specific cumulative process evolution.

Claim 7: stronger future claim. A human-free recursive AI system would require
replacing human selection with protected artificial consequence, not merely
automating research labor.
...
A system can show local improvement without satisfying the conditions for
autonomous recursive accumulation. For accumulation to become recursive and
self-sustaining, at least the following conditions are required.

Stable interpreter. There must be a relatively stable class of machinery capable
of reconstructing process from inherited structure. If the interpreter changes
too quickly, inherited artifacts lose meaning.

Persistent substrate. There must be a medium in which process-relevant
structures can survive beyond a single episode, organism, mind, or rollout.

Transmission gap. Internal state must not carry over automatically. A gap must
force useful process to be externalized, compressed, and reconstructed by later
instances.

Heritable variation. Inherited structures must vary, and descendants must
resemble ancestors enough for successful variants to have lineage.

Part-level visibility. Useful substructures must become identifiable enough to
be copied, modified, recombined, tested, or discarded. Whole-system success
alone gives weak accumulation.

Scarcity. Attention, context, compute, continuation, or visibility must be
limited. Without scarcity, poor artifacts persist alongside useful ones and
selection weakens.

Protected selection. The current variant must not be able to freely redefine the
criterion by which its own continuation is judged. Some consequence layer must
remain fixed or slow-changing.

Differential continuation. Structures that improve future success, robustness,
efficiency, or option value must receive more future influence than structures
that do not.

Downstream uptake. Artifacts must be evaluated primarily by later use, not
producer self-description. Exposure and mention are weak signals; adoption,
dependency, modification, and ablation-sensitive contribution are stronger
signals.

Plasticity-persistence balance. The system must allow enough mutation and
recombination to discover improvements, but enough fidelity and stability to
retain them.

A compact formal condition can be stated as follows. Let `a_i` be an inherited
artifact or process structure, and let `q_i(t)` be its future representation:
probability of retrieval, inclusion in seeds, dependency status, lineage
continuation, or compute allocation. A minimal accumulation condition is:
...
If useful process structures do not gain future representation because of their
usefulness, then the system may store, imitate, or report artifacts, but it is
not accumulating adaptively.

### Persistence-Plasticity Constraint

| System | Plastic interpreter | Persistent substrate | Selection layer | Failure if missing |
| --- | --- | --- | --- | --- |
| Biology | organism and cell activity | genome and inherited cellular organization | environment-mediated reproduction | no cumulative adaptation |
| Human culture | brains, social learning, institutions | speech, writing, tools, practices, records | material, social, and institutional uptake | isolated learning, weak accumulation |
| Current AI labs | models plus human researchers | code, data, evals, checkpoints, docs | human and institutional judgment plus benchmarks | hybrid improvement, not autonomous recursive self-improvement |
...
The proposed architecture is not trying to mimic DNA literally. It is trying to
satisfy the same persistence-plasticity constraint at the process level. Fresh
rollouts supply plasticity. Seeds and archives supply persistence. Primitive
physics and uptake supply selection. Bounded context and fresh starts create the
transmission gap that makes externalized process necessary.

### Automating Labor Is Not Automating Selection

A common path to recursive AI improvement imagines a smooth reduction of human
involvement: first humans conduct AI research, then AI assists them, then AI
performs most research labor, and eventually AI performs all of it. This picture
treats human contribution mainly as labor.

But in cumulative systems, the crucial role is often not labor but selection.
Variation is cheap relative to knowing what should persist. A system can
generate many candidate changes without possessing an autonomous criterion for
which changes improve future continuation.

Human researchers currently supply more than effort. They stabilize the meaning
of improvement. They choose benchmarks, interpret anomalies, detect Goodharting,
define safety constraints, judge whether data is meaningful, decide whether an
experiment should be scaled, and preserve lessons in institutional form. In this
sense, humans are not merely doing work inside the loop; they are part of the
loop's fitness function.

Eliminating humans therefore requires more than automating the tasks humans
perform. It requires replacing the human selection layer with a different
structure. The replacement cannot simply be self-rating, because a system that
freely defines its own success can preserve artifacts that satisfy its own
evaluator without improving against any external or protected constraint.
Autonomous recursive accumulation requires a selection function that the current
variant cannot freely rewrite.

This is why the proposed architecture emphasizes primitive physics: fresh
starts, bounded context, bounded inheritance, costly attention, uneditable
current-run scoring, delayed descendant evaluation, archive uptake, and
differential continuation. These constraints do not define a mature culture.
They define the non-negotiable consequence layer within which artificial culture
can evolve.

### What Would Count as Removing the Human Selector?

A system has not eliminated human selection merely because fewer humans write
code, inspect logs, or manually curate data. Human selection remains if humans
still define the operative success criteria, choose which evaluators matter,
decide which failures are meaningful, approve successor changes, or determine
what counts as progress.

A stronger threshold is required. Human selection has been replaced only when
candidate process structures can be generated, tested, inherited, recombined,
and differentially preserved by a system-internal ecology whose primitive
consequences are not freely rewritten by the current variant.

This does not require eliminating all human-defined outer constraints. Biology
operates under physical law; culture operates under material and social
consequence. An artificial system may operate under human-initialized primitive
physics: fixed task interfaces, compute costs, verifier rules, safety
boundaries, or deployment feedback. But within those boundaries, process
artifacts must compete through downstream usefulness rather than manual
approval.

This defines the transition from AI-assisted human cultural evolution to
AI-native cultural evolution.
...
Conditions for Language Emergence:
